...
- Продолжаем тупо копировать файлы `app/Http/Controllers/API/NotificationController.php` вызывается через через `api.php` принимает в качестве запроса массив id пользователей, заголовок уведомления и сам текст уведомления. Далее находит соответствующие данным пользователям записи в базе данных используя модель `NotificationClient`. Затем по данным из возврата из fcm->sendToTokens удаляет все неактуальные токены из базы. А еще мы поняли что не сможем простить себе, если оставим вам в api.php ту гадость, поэтому во время написания мы еще раз отрефакторили код и теперь saveFcmToken, который принимает собственно сам FCM токен и название платформы, которые помещает в базу данных находится в этом файле, кроме того тут обрабатывается данная возможная ошибка, если вы ее еще помните:
     		...
     		- Для этого для начала скопируйте файл `public\firebase-messaging-sw.js`
       		- Затем получите на https://console.firebase.google.com новый конфиг таким образом:
				...
				- вставьте в файл `public\firebase-messaging-sw.js` аккуратно именно внутрь блока firebase.initializeApp
         		- Необходимо зайти в `nginx.conf` и в блок `server {` вставить вот это:
           		- ```
                  ...
                  ```
               - Если не углублятся в детали это позволит service worker работать корркетно, отсутствия кэширования, чтобы уменьшить проблемы при отладке, дает возможность service worker контролировать весь сайт, а не только папку, делает доступным статически напрямую
